...
int numofblock(int** map, int center_x, int center_y) {
    ...
    for (int i = 0; i < 3; i++) for (int j = 0; j < 3; j++)if (searched[i][j] == false) {
        searched[i][j] = true; if (tempmap[i][j] == 0) cnt++;
...
int create_mnt(int** map, int x, int y) {
    int tp = 0;
    if (map[x][y] == 0) if (appendable(map, x, y)) {
        if (rand() % 100 > CITY_PERCENTAGE) map[x][y] = -1;
        else map[x][y] = 1;
        tp = 1;
    }
...
int** generatemap(int x, int y, int player_num) {
